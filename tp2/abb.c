...
bool _abb_in_order(abb_nodo_t *nodo, bool visitar(const char *, void *, void *), void *extra, const char *desde, const char *hasta, abb_comparar_clave_t cmp) {
	if (!nodo) return true;
	
	if (desde[0] == '\0' || cmp(nodo->clave, desde) >= 0) {
		if(!_abb_in_order(nodo->izq, visitar, extra, desde, hasta, cmp)) return false;
	}
	if (visitar && (desde[0] == '\0' || cmp(nodo->clave, desde) >= 0) && (hasta[0] == '\0' || cmp(nodo->clave, hasta) <= 0)) {
		if (!visitar(nodo->clave, nodo->dato, extra)) return false;
	}
	if (hasta[0] == '\0' || cmp(nodo->clave, hasta) <= 0) {
		return _abb_in_order(nodo->der, visitar, extra, desde, hasta, cmp);
	} else {
		return false;
	}
}
...
void abb_in_order(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra, const char *desde, const char *hasta) {
	...
	_abb_in_order(arbol->raiz, visitar, extra, desde, hasta, arbol->cmp);
}
